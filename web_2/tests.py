...
class ViewsTestCase(TestCase):
    ...
    def test_webscraping_view_get(self):
        """Test WebScraping view GET request"""
        response = self.client.get(reverse("webscraping"))  # Add urlpattern name
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "home.html")

    def test_index_view_get(self):
        """Test IndexView GET request"""
        response = self.client.get(reverse("index"))  # Add urlpattern name
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "page/index.html")

    @patch("yourapp.services.YouTubeCommentService")
    def test_commentfetch_view_post_valid_url(self, MockYTService):
        ...
        response = self.client.post(reverse("fetch_comments"), {
    ...
    def test_commentfetch_view_get(self):
        """Test CommentFetchView GET request"""
        response = self.client.get(reverse("fetch_comments"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "page/index.html")
